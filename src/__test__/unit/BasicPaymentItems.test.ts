...
const items = new BasicPaymentItems(
  new BasicPaymentProducts({ paymentProducts }),
);
...
it.each(paymentProductIds)(
  '`basicPaymentItemById[%i]` should contain corresponding BasicPaymentProduct` ',
  (id) => {
...
it('should have correct `accountsOnFileById`', () => {
  ...
  [1, 2, 3].forEach((id) =>
    expect(items.accountOnFileById[id]?.id).toBe(Number(id)),
  );
});
